...
class QueueController
{
    ...
    public function store()
    {
        ...
        $publishModes = $_POST['publish_mode'] ?? [];
        $targetLanguages = $_POST['target_language'] ?? [];
        ...
        // Obtener idioma del post original
        $sourceLanguage = Network::getConfig($tenantId)->default_language ?? 'es';

        $targets = [];
        foreach ($targetTenantIds as $targetId) {
            // Verificar que no esté ya en cola
            if (!Queue::isQueued($postId, $targetId)) {
                $mode = $publishModes[$targetId] ?? 'clone';
                $targetLang = $targetLanguages[$targetId] ?? 'es';
                $needsTranslation = ($targetLang !== $sourceLanguage);

                $targets[] = [
                    'tenant_id' => (int) $targetId,
                    'language' => $targetLang,
                    'translate' => $needsTranslation,
                    'adapt_style' => ($mode === 'adapt')
                ];
            }
        }
