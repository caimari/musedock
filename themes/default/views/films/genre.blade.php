...
<div class="container py-5">
  <div class="text-center mb-5">
    ...
    @if($genre->description)
      <p class="lead text-muted">{{ $genre->description }}</p>
    @endif
  </div>

  <p class="text-muted mb-3">{{ $pagination['total_posts'] ?? 0 }} películas</p>
      ...
      <a href="/films/{{ $film->slug }}" class="text-decoration-none">
        <div class="card h-100 border-0 shadow-sm film-card">
          ...
          <div class="card-body p-2">
            ...
            <small class="text-muted">{{ $film->year ?? '' }} · {{ $film->director ?? '' }}</small>
          </div>
        </div>
      </a>
    </div>
    ...
    <ul class="pagination">
      @for($p = 1; $p <= $pagination['total_pages']; $p++)
        <li class="page-item {{ $p == $pagination['current_page'] ? 'active' : '' }}">
          <a class="page-link" href="/films/genero/{{ $genre->slug }}?page={{ $p }}">{{ $p }}</a>
        </li>
      @endfor
    </ul>
  </nav>
