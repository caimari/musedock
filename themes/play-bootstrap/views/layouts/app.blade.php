...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('themes/play-bootstrap/css/ud-styles.css') }}?v=9" />
...
<body>
    ...
    @php
        $__pbSearchEnabled = themeOption('header.header_search_enabled', false);
        $__pbSearchMode = themeOption('header.header_search_mode', 'modal');
        $__pbSearchLang = detectLanguage();
    @endphp
    @if($__pbSearchEnabled && $__pbSearchMode === 'modal')
        ...
        <div class="md-search-overlay-inner">
            ...
            <form action="{{ url('/search') }}" method="GET" class="md-search-form">
                <div class="md-search-input-wrap">
                    ...
                    <input type="text" name="q" class="md-search-input" placeholder="{{ $__pbSearchLang === 'en' ? 'Search...' : 'Buscar...' }}" autocomplete="off" minlength="2">
                </div>
                <p class="md-search-hint">{{ $__pbSearchLang === 'en' ? 'Press Enter to search or Esc to close' : 'Pulsa Enter para buscar o Esc para cerrar' }}</p>
            </form>
        </div>
    </div>
    <style>
    ...
    .md-search-overlay.active { opacity: 1; visibility: visible; }
    .md-search-overlay-inner { width: 100%; max-width: 640px; padding: 0 20px; position: relative; }
    .md-search-close {
        position: absolute; top: -60px; right: 20px;
        background: none; border: none; color: rgba(255,255,255,0.6); cursor: pointer; padding: 8px; transition: color 0.2s;
    }
    .md-search-close:hover { color: #fff; }
    .md-search-input-wrap { position: relative; display: flex; align-items: center; }
    .md-search-icon { position: absolute; left: 20px; color: rgba(255,255,255,0.4); pointer-events: none; }
    .md-search-input {
        width: 100%; padding: 18px 20px 18px 56px; font-size: 20px; font-weight: 400; color: #fff;
    ...
    .md-search-input::placeholder { color: rgba(255,255,255,0.35); }
    .md-search-input:focus { border-color: rgba(255,255,255,0.3); background: rgba(255,255,255,0.12); }
    .md-search-hint { text-align: center; color: rgba(255,255,255,0.35); font-size: 13px; margin-top: 14px; }
    .header-search-toggle {
        background: none; border: none; cursor: pointer; padding: 6px; margin-right: 10px;
        color: var(--header-link-color, #fff); display: inline-flex; align-items: center;
        justify-content: center; transition: color 0.2s; vertical-align: middle; text-decoration: none;
    }
    .header-search-toggle:hover { color: var(--header-link-hover-color, rgba(255,255,255,0.7)); }
