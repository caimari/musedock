...
class CreateSuperadminPluginLogsTable_2025_12_11_091120
{
    public function up()
    {
  ...
  CHECK (level IN ('info', 'warning', 'error', 'debug'))
            ...
            // Create indexes
            $pdo->exec("CREATE INDEX plugin_id ON superadmin_plugin_logs(plugin_id)");
            $pdo->exec("CREATE INDEX level ON superadmin_plugin_logs(level)");
            $pdo->exec("CREATE INDEX created_at ON superadmin_plugin_logs(created_at)");
