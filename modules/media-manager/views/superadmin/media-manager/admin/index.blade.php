...
<div class="app-content">
    <div class="container-fluid">
        ...
        {{-- Título y Selector de Disco --}}
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h2>{{ $title }}</h2>

            {{-- Selector de Disco --}}
            @if(count($availableDisks ?? []) > 1)
            <div class="disk-selector d-flex align-items-center gap-2">
                <label class="text-muted small mb-0">Almacenamiento:</label>
                <select id="disk-selector" class="form-select form-select-sm" style="width: auto; min-width: 180px;">
                    @foreach($availableDisks as $diskKey => $diskInfo)
                    <option value="{{ $diskKey }}" data-icon="{{ $diskInfo['icon'] }}" {{ ($defaultDisk ?? 'media') === $diskKey ? 'selected' : '' }}>
                        {{ $diskInfo['name'] }}
                    </option>
                    @endforeach
                </select>
                <span id="disk-status" class="badge bg-success" title="Conectado">
        ...
        <div id="media-uploader" class="mb-4">
            <form id="upload-form" action="{{ route('superadmin.media.upload') }}" method="POST" enctype="multipart/form-data">
                 ...
                 <input type="hidden" name="disk" id="upload-disk" value="{{ $defaultDisk ?? 'media' }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const uploadDiskInput = document.getElementById('upload-disk');

    if (diskSelector) {
        diskSelector.addEventListener('change', function() {
            const selectedDisk = this.value;
            window.MediaManagerConfig.currentDisk = selectedDisk;

            // Actualizar el campo oculto del formulario de upload
            if (uploadDiskInput) {
                uploadDiskInput.value = selectedDisk;
            }

            // Recargar la biblioteca de medios con el nuevo disco
            if (typeof window.loadMedia === 'function') {
                window.loadMedia(1);
            }

            // Recargar estructura de carpetas del nuevo disco
            if (window.FolderManager && typeof window.FolderManager.loadFolderStructure === 'function') {
                window.FolderManager.currentFolderId = null; // Reset a raíz
                window.FolderManager.loadFolderStructure();
            }

            // Actualizar indicador de estado
            const diskStatus = document.getElementById('disk-status');
            if (diskStatus) {
                diskStatus.innerHTML = '<i class="bi bi-arrow-repeat spin"></i>';
                diskStatus.className = 'badge bg-warning';

                // Simular verificación de conexión
                setTimeout(() => {
                    diskStatus.innerHTML = '<i class="bi bi-check-circle"></i>';
                    diskStatus.className = 'badge bg-success';
                }, 500);
            }
        });
    }
