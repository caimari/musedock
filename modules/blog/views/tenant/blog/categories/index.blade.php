...
<div class="app-content">
  <div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <a href="{{ admin_url('blog/categories/create') }}" class="btn btn-primary">{{ __('blog.category.add_category') }}</a>
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      <form method="GET" action="{{ admin_url('blog/categories') }}" class="d-flex align-items-center">
        ...
        @if (!empty($search))
          <a href="{{ admin_url('blog/categories') }}" class="btn btn-outline-danger btn-sm">{{ __('common.clear_filter') }}</a>
        @endif
      </form>
    </div>

    {{-- Formulario Acciones en Lote --}}
    <form method="POST" action="{{ admin_url('blog/categories/bulk') }}" id="bulkActionForm">
      @csrf
      <div class="card">
            ...
            @foreach ($categories as $category)
              <tr data-id="{{ $category->id }}">
                ...
                <td>
                  ...
                  <br>
                  <small>
                    <a href="{{ admin_url('blog/categories/' . $category->id . '/edit') }}">{{ __('common.edit') }}</a>
                     |
          ...
          @else
            <div class="p-3 text-center">
              <p class="text-muted">{{ __('blog.category.no_categories_found') }}</p>
               @if(empty($search))
                 <a href="{{ admin_url('blog/categories/create') }}" class="btn btn-sm btn-primary">{{ __('blog.category.create_first_category') }}</a>
               @endif
            </div>
          @endif
        </div>
