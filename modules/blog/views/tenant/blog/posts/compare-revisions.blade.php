...
<div class="d-flex justify-content-between align-items-center mb-3"><h2>{{ $title }}</h2><a href="{{ route('tenant.blog.posts.revisions', $post->id) }}" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Volver a revisiones</a></div>
<div class="card mb-3"><div class="card-body"><h5>{{ e($post->title) }}</h5></div></div>
<div class="diff-container">
<div class="diff-column"><h6 class="mb-3">Revisión #{{ $revision1->id }}<small class="text-muted ms-2">{{ date('d/m/Y H:i', strtotime($revision1->created_at)) }}</small></h6>
...
<div class="diff-column"><h6 class="mb-3">Revisión #{{ $revision2->id }}<small class="text-muted ms-2">{{ date('d/m/Y H:i', strtotime($revision2->created_at)) }}</small></h6>
...
<div class="card mt-3"><div class="card-body"><h6>Resumen de cambios:</h6><ul>
...
@if(!$diff['title'] && !$diff['content'])<li class="text-muted">Sin cambios detectados</li>@endif
</ul></div></div>
</div></div>
@endsection
