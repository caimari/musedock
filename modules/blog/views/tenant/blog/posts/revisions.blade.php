...
@section('styles')
<style>
  .revision-row { cursor: pointer; transition: background-color 0.2s; }
  .revision-row:hover { background-color: #f8f9fa; }
  .revision-type-badge { font-size: 0.75rem; padding: 0.25rem 0.5rem; }
...
<div class="app-content">
  <div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h2>{{ $title }}</h2>
      <div>
        <a href="{{ route('tenant.blog.posts.edit', $post->id) }}" class="btn btn-secondary me-2">
          <i class="bi bi-arrow-left"></i> Volver a editar
        </a>
        <a href="{{ route('tenant.blog.posts.index') }}" class="btn btn-outline-secondary">
          ...
          <table class="table table-hover">
            ...
            <tbody>
              ...
              <tr class="revision-row">
                ...
                <td>
                  @php
                    $badges = ['initial' => ['success', 'Inicial'], 'manual' => ['primary', 'Manual'], 'autosave' => ['info', 'Autoguardado'], 'published' => ['warning', 'Publicado'], 'restored' => ['dark', 'Restaurado']];
                ...
                <td>
                  <div class="btn-group btn-group-sm">
                    <a href="{{ route('tenant.blog.posts.revisions.preview', [$post->id, $revision->id]) }}" class="btn btn-outline-secondary" title="Vista previa"><i class="bi bi-eye"></i></a>
                    <form method="POST" action="{{ route('tenant.blog.posts.revisions.restore', [$post->id, $revision->id]) }}" style="display:inline;" onsubmit="return confirm('¿Restaurar el post a esta versión?');">
                      <input type="hidden" name="_token" value="{{ csrf_token() }}">
                      <button type="submit" class="btn btn-outline-primary" title="Restaurar"><i class="bi bi-arrow-counterclockwise"></i></button>
                    </form>
                  </div>
                </td>
              </tr>
...
function toggleCompareButton() {
  ...
  if (selectedRevisions.length === 2) {
    document.querySelectorAll('.compare-checkbox:not(:checked)').forEach(cb => cb.disabled = true);
  } else {
    document.querySelectorAll('.compare-checkbox').forEach(cb => cb.disabled = false);
  }
}
function compareSelected() {
  if (selectedRevisions.length === 2) {
    @php $adminPath = $_SESSION['admin']['tenant_url'] ?? 'admin'; @endphp
    window.location.href = `/${!! json_encode($adminPath) !!}/blog/posts/{{ $post->id }}/revisions/${selectedRevisions[0]}/compare/${selectedRevisions[1]}`;
  }
}
</script>
@endsection
