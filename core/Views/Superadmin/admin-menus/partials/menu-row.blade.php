...
<tr data-id="{{ $menu->id }}" class="menu-row-level-{{ $level }}">
  ...
  <td>
    @if($menu->icon)
      <i class="{{ $menu->icon_type === 'bi' ? 'bi' : $menu->icon_type }} {{ $menu->icon_type === 'bi' ? 'bi-' . $menu->icon : 'fa-' . $menu->icon }}"></i>
  ...
  <td>
    <a href="{{ route('admin-menus.edit', ['id' => $menu->id]) }}" class="btn btn-sm btn-outline-primary" title="Editar">
      <i class="bi bi-pencil"></i>
    </a>
    <button class="btn btn-sm btn-outline-danger delete-menu-btn"
            data-id="{{ $menu->id }}"
            data-title="{{ htmlspecialchars($menu->title, ENT_QUOTES, 'UTF-8') }}"
            title="Eliminar">
