...
{{-- Versión con Media Manager integrado --}}
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        @php
            ...
            if (!empty($sliderImage)) {
                ...
                if (str_starts_with($sliderImage, '/media/') || str_starts_with($sliderImage, 'http')) {
                    $imageUrl = $sliderImage;
                } else {
                    // Ruta de assets local
                    $imageUrl = asset($sliderImage);
                }
            } else {
                $isCustom = false;
        ...
        <div class="mb-3" id="slider-image-container" style="{{ old($toggleName, $Page->{$toggleName} ?? 0) == 1 ? '' : 'display: none;' }}">
            ...
            <input type="hidden" name="{{ $imageField }}" id="header-image-input" value="{{ $sliderImage }}">
            ...
            <small class="text-muted d-block mt-2">
                <i class="bi bi-info-circle"></i> Recomendado: 1920x400px para páginas, 1920x600px para blog.
            </small>
        </div>
