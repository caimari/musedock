...
<h2>{{ $title }}</h2>
<a href="/musedock/pages" class="btn btn-outline-secondary"><i class="bi bi-list"></i> Volver a lista de páginas</a>
</div>
...
<form method="POST" action="/musedock/pages/{{ $page->id }}/force-delete" style="display:inline;" onsubmit="return confirm('¿ELIMINAR PERMANENTEMENTE esta página?');">
<input type="hidden" name="_token" value="{{ csrf_token() }}">
