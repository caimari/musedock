...
<div class="row">
    <div class="col-md-8">
        ...
        <div class="mb-3">
            ...
            @else
                <small class="text-warning">
                    ...
                    <a href="/{{ admin_path() }}/modules">Activar módulo</a>
                </small>
            @endif
        ...
        <div class="mb-3">
            <label class="form-label">Punto focal de la imagen</label>
            <small class="d-block text-muted mb-2">Haz clic en la imagen para elegir qué zona se muestra cuando el slider recorta la foto.</small>
            <div id="focal-point-container" style="position:relative;display:block;max-width:100%;cursor:crosshair;border:2px solid #e0e0e0;border-radius:6px;overflow:hidden;user-select:none;">
                <img id="focal-preview" src="{{ old('image_url', $slide->image_url ?? '/assets/superadmin/img/placeholder-image.png') }}" alt="Focal point" draggable="false" style="display:block;width:100%;max-height:300px;object-fit:contain;pointer-events:none;">
                <div id="focal-marker" style="position:absolute;width:24px;height:24px;border:3px solid #fff;border-radius:50%;box-shadow:0 0 0 2px rgba(0,0,0,0.6),0 2px 8px rgba(0,0,0,0.4);transform:translate(-50%,-50%);pointer-events:none;z-index:10;background:rgba(59,130,246,0.4);left:50%;top:50%;"></div>
            </div>
            <input type="hidden" name="image_position" id="image_position" value="{{ old('image_position', $slide->image_position ?? 'center center') }}">
            <div class="mt-2">
                <small class="text-muted">Posición actual: <strong id="focal-label">{{ old('image_position', $slide->image_position ?? 'center center') }}</strong></small>
                <button type="button" class="btn btn-sm btn-outline-secondary ms-2" id="focal-reset">Centrar</button>
            </div>
        </div>
...
<div class="mt-4">
    ...
    <a href="/{{ admin_path() }}/sliders/{{ $slide->slider_id ?? $sliderId }}/edit" class="btn btn-secondary">Cancelar</a>
</div>

{{-- Script para actualizar preview y punto focal --}}
<script>
document.addEventListener('DOMContentLoaded', function() {
    const imageUrlInput = document.getElementById('image_url');
    const imagePreview = document.getElementById('image_preview');
    const focalPreview = document.getElementById('focal-preview');
    const focalContainer = document.getElementById('focal-point-container');
    const focalMarker = document.getElementById('focal-marker');
    const focalInput = document.getElementById('image_position');
    const focalLabel = document.getElementById('focal-label');
    const focalReset = document.getElementById('focal-reset');

    // Sincronizar imagen URL con ambas previews
    if (imageUrlInput && imagePreview) {
        imageUrlInput.addEventListener('input', function() {
            const url = this.value.trim();
            if (url) {
                imagePreview.src = url;
                if (focalPreview) focalPreview.src = url;
            } else {
                imagePreview.src = '/assets/superadmin/img/placeholder-image.png';
                if (focalPreview) focalPreview.src = '/assets/superadmin/img/placeholder-image.png';
            }
        });
    ...
    if (focalPreview) {
        focalPreview.addEventListener('error', function() {
            this.src = '/assets/superadmin/img/placeholder-image.png';
        });
    }

    // Focal point picker
    function setFocalPoint(xPercent, yPercent) {
        var xR = Math.max(0, Math.min(100, Math.round(xPercent)));
        var yR = Math.max(0, Math.min(100, Math.round(yPercent)));
        var val = xR + '% ' + yR + '%';
        if (focalInput) focalInput.value = val;
        if (focalLabel) focalLabel.textContent = val;
        if (focalMarker) {
            focalMarker.style.left = xR + '%';
            focalMarker.style.top = yR + '%';
        }
    }

    // Inicializar marcador con el valor guardado
    if (focalInput && focalInput.value) {
        var parts = focalInput.value.replace(/%/g, '').trim().split(/\s+/);
        if (parts.length === 2) {
            setFocalPoint(parseFloat(parts[0]), parseFloat(parts[1]));
        }
    }

    // Click/drag en el container para mover el punto focal
    if (focalContainer) {
        var isDragging = false;

        function handleFocalMove(e) {
            e.preventDefault();
            var rect = focalContainer.getBoundingClientRect();
        ...
        focalContainer.addEventListener('mousedown', function(e) {
            isDragging = true;
            handleFocalMove(e);
        });
        document.addEventListener('mousemove', function(e) {
            if (isDragging) handleFocalMove(e);
        });
        ...
        // Touch support
        focalContainer.addEventListener('touchstart', function(e) {
            isDragging = true;
            handleFocalMove(e);
        }, {passive: false});
    ...
    // Reset
    if (focalReset) {
        focalReset.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            setFocalPoint(50, 50);
        });
...
if (window.MediaManagerConfig.foldersStructureUrl.includes('#ruta-no-encontrada')) {
    window.MediaManagerConfig.foldersStructureUrl = '/{{ admin_path() }}/media/folders/structure';
}
if (window.MediaManagerConfig.uploadUrl.includes('#ruta-no-encontrada')) {
    window.MediaManagerConfig.uploadUrl = '/{{ admin_path() }}/media/upload';
}
if (window.MediaManagerConfig.dataUrl.includes('#ruta-no-encontrada')) {
    window.MediaManagerConfig.dataUrl = '/{{ admin_path() }}/media/data';
}
if (window.MediaManagerConfig.deleteUrlTemplate.includes('#ruta-no-encontrada')) {
    window.MediaManagerConfig.deleteUrlTemplate = '/{{ admin_path() }}/media/:id/delete';
}
if (window.MediaManagerConfig.detailsUrlTemplate.includes('#ruta-no-encontrada')) {
    window.MediaManagerConfig.detailsUrlTemplate = '/{{ admin_path() }}/media/:id/details';
}
if (window.MediaManagerConfig.updateUrlTemplate.includes('#ruta-no-encontrada')) {
    window.MediaManagerConfig.updateUrlTemplate = '/{{ admin_path() }}/media/:id/update';
}
</script>
