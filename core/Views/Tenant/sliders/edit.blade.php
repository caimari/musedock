...
<div class="app-content">
    <div class="container-fluid">
        ...
        <form method="POST" action="/{{ admin_path() }}/sliders/{{ $slider->id }}/update">
	            @csrf
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <a href="/{{ admin_path() }}/sliders/{{ $slider->id }}/slides/create" class="btn btn-sm btn-primary">
                        ...
                        <div class="card-body table-responsive p-0">
                                    ...
                                    <tbody id="slides-list-{{ $slider->id }}">
                                        @foreach($slides as $slide)
                                            <tr data-slide-id="{{ $slide->id }}">
                                                ...
                                                <td class="text-end">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="/{{ admin_path() }}/sliders/slides/{{ $slide->id }}/edit" class="btn btn-outline-primary" title="Editar Diapositiva">
                            ...
                            @else
                                <div class="p-3 text-center text-muted">
                                    Este slider aún no tiene diapositivas. <a href="/{{ admin_path() }}/sliders/{{ $slider->id }}/slides/create">Añade la primera</a>.
                                </div>
                            @endif
                        </div>
                    </div>
...
@if(isset($slides) && count($slides) > 0)
    @foreach($slides as $slide)
        <form id="delete-slide-form-{{ $slide->id }}" action="/{{ admin_path() }}/sliders/slides/{{ $slide->id }}/destroy" method="POST" style="display: none;">
            @csrf
        </form>
    @endforeach
@endif
...
function updateSlideOrder() {
    ...
    // Enviar al servidor
    fetch('/{{ admin_path() }}/sliders/{{ $slider->id }}/slides/order', {
        method: 'POST',
        headers: {
