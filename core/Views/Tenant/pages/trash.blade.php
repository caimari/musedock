...
<h2>{{ $title }}</h2>
<a href="{{ admin_url('pages') }}" class="btn btn-outline-secondary"><i class="bi bi-list"></i> {{ __('pages.back_to_pages_list') }}</a>
</div>
...
<form method="POST" action="{{ admin_url('pages') }}/{{ $page->id }}/force-delete" style="display:inline;" onsubmit="return confirm('{{ __('pages.confirm_permanent_delete') }}');">
<input type="hidden" name="_token" value="{{ csrf_token() }}">
