{{-- Shared appearance form partial - used by both Tenant and Superadmin --}}
@php
    // Fallback: if $__adminPath not set by parent view, detect context
    if (!isset($__adminPath) || empty($__adminPath)) {
        $__adminPath = $adminBasePath ?? (function_exists('admin_path') ? admin_path() : 'admin');
    }
@endphp
...
<div class="app-content">
    <div class="container-fluid">
                ...
                <nav aria-label="breadcrumb" class="mb-0">
                    <ol class="breadcrumb mb-0 align-items-center" style="flex-wrap: nowrap; white-space: nowrap;">
                        <li class="breadcrumb-item">
                            <a href="/{{ $__adminPath }}/themes"><i class="fas fa-palette me-1"></i>{{ __('themes.title') ?? 'Temas' }}</a>
                        </li>
        ...
        <div class="row">
            <!-- Panel de opciones -->
            <div class="col-lg-8">
                <form method="POST" action="/{{ $__adminPath }}/themes/appearance/{{ $slug }}/save" id="appearanceForm">
                    {!! csrf_field() !!}
                    ...
                    @foreach ($optionsSchema as $sectionSlug => $section)
                        ...
                        <div class="appearance-section card mb-4" id="section-{{ $sectionSlug }}"
                             @if(!empty($section['depends_on']))
                                 data-section-depends="{{ $section['depends_on'] }}"
                                 style="{{ array_check_dependency($savedOptions ?? [], $section['depends_on'] ?? null) ? '' : 'display: none;' }}"
                             @endif>
                                    ...
                                    @foreach ($section['options'] as $optionSlug => $option)
                                        ...
                                        <div class="{{ $colClass }} mb-3 option-group"
                                            ...
                                            @switch($option['type'] ?? 'text')
                                                ...
                                                @case('link')
                                                    @php
                                                        $linkUrl = str_replace('{admin_path}', $__adminPath, $option['url'] ?? '#');
                                                    @endphp
                ...
                <form method="POST" action="/{{ $__adminPath }}/themes/appearance/{{ $slug }}/reset" id="resetForm" style="display: none;">
                    {!! csrf_field() !!}
                </form>
            </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-footer bg-light">
                        <div class="d-flex gap-2">
                            <a href="/{{ $__adminPath }}/themes/appearance/{{ $slug }}/export" class="btn btn-sm btn-outline-secondary flex-grow-1">
...
<form method="POST" action="/{{ $__adminPath }}/themes/appearance/{{ $slug }}/preset/load/" id="loadPresetForm" style="display: none;">
    {!! csrf_field() !!}
</form>

<form method="POST" action="/{{ $__adminPath }}/themes/appearance/{{ $slug }}/import" id="importPresetForm" enctype="multipart/form-data" style="display: none;">
    {!! csrf_field() !!}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkDependency(dependsOn) {
        ...
        const driverElement = document.querySelector(`[id*="option-${driverKey.replace(/\./g, '-')}"]`);
        if (!driverElement) return true;

        if (expectedValue !== null) {
            return driverElement.value === expectedValue;
        } else if (driverElement.type === 'checkbox') {
            return driverElement.checked;
        } else {
            return Boolean(driverElement.value) && driverElement.value !== '0';
        }
    }
    ...
    function updateDependencies() {
        ...
        document.querySelectorAll('.option-group[data-depends-on]').forEach(group => {
            group.style.display = checkDependency(group.dataset.dependsOn) ? '' : 'none';
        });

        // Section-level dependencies (hide/show entire sections)
        document.querySelectorAll('.appearance-section[data-section-depends]').forEach(section => {
            section.style.display = checkDependency(section.dataset.sectionDepends) ? '' : 'none';
        });
    }
    ...
    // ==================== MEDIA MANAGER (Image picker) ====================
    document.querySelectorAll('.btn-media-manager').forEach(btn => {
        btn.addEventListener('click', function() {
            const inputId = this.dataset.inputId;
            const input = document.getElementById(inputId);
            if (!input) return;

            // Abrir media manager en un modal/popup
            const mediaUrl = '/' + adminPath + '/media?picker=1&type=image';
            const mediaWindow = window.open(mediaUrl, 'MediaManager', 'width=900,height=600,scrollbars=yes');

            // Escuchar mensaje del media manager
            window.addEventListener('message', function handler(e) {
                if (e.data && e.data.type === 'media-selected' && e.data.url) {
                    input.value = e.data.url;
                    // Actualizar preview si existe
                    const preview = input.closest('.input-group')?.nextElementSibling;
