...
class TenantPluginManager
{
    ...
    /**
     * Obtener todos los directorios de plugins compartidos (local + privado externo).
     *
     * @return array Lista de rutas absolutas
     */
    public static function getAllSharedPluginsPaths(): array
    {
        $paths = [self::getSharedPluginsPath()];

        $privatePath = function_exists('env') ? env('PRIVATE_PLUGINS_PATH') : ($_ENV['PRIVATE_PLUGINS_PATH'] ?? null);
        if ($privatePath) {
            $externalDir = rtrim($privatePath, '/') . '/tenant-shared';
     ...
     * luego en los directorios de plugins compartidos (local + privado).
     *
     * @param int $tenantId ID del tenant
     * @param string $slug Slug del plugin
    ...
    public static function resolvePluginPath(int $tenantId, string $slug): ?string
    {
        ...
        // 2. Buscar en todos los directorios de tenant-shared (local + privado)
        foreach (self::getAllSharedPluginsPaths() as $sharedDir) {
            $sharedPath = $sharedDir . '/' . $slug;
