...
protected static function renderSwiper(int $sliderId, array $slides, array $settings): string
{
    ...
    if ($fullWidth) {
        ...
        $output .= '<div class="slider-full-width-wrapper" data-slider-id="' . $sliderId . '" style="position:relative !important;left:50% !important;right:50% !important;margin-left:-50vw !important;margin-right:-50vw !important;width:100vw !important;max-width:100vw !important;box-sizing:border-box !important;margin-top:0 !important;">';

        $output .= '<script>
        (function(){
            var wrapper = document.querySelector(\'.slider-full-width-wrapper[data-slider-id="' . $sliderId . '"]\');
            if(wrapper){
                document.body.style.overflowX = "clip";
                function fixWidth() {
                    var vw = document.documentElement.clientWidth;
                    wrapper.style.width = vw + "px";
                    wrapper.style.marginLeft = "-" + (vw/2) + "px";
                }
                fixWidth();
    ...
    foreach ($slides as $slide) {
        ...
        $objPos = isset($slide->image_position) && trim($slide->image_position) !== '' ? trim($slide->image_position) : 'center center';
        $imgStyle = 'width:100%;height:100%;object-fit:cover;object-position:' . \e($objPos) . ' !important;';
        $bgStyle = 'position:absolute;top:0;left:0;width:100%;height:100%;background-size:cover;background-position:' . \e($objPos) . ';background-repeat:no-repeat;';
        if ($theme === 'rounded-shadow') {
            $slideStyle .= 'border-radius:22px !important;overflow:hidden !important;';
            $imgStyle .= 'border-radius:22px !important;';
            $bgStyle .= 'border-radius:22px !important;';
        }
        $output .= '<div class="swiper-slide" style="' . $slideStyle . '">';

        // Portrait-carousel: imagen con lightbox + caption debajo (mantener <img> para lightbox)
        ...
        // Usar div con background-image para control robusto de posición focal
        $output .= '<div style="' . $bgStyle . 'background-image:url(\'' . \e($slide->image_url) . '\');"></div>';
        // Mantener img oculta para accesibilidad/SEO
        $output .= '<img src="' . \e($slide->image_url) . '" alt="" style="display:none;">';
    ...
    protected static function renderGallery(int $sliderId, array $slides, array $settings): string
    {
        ...
        $output .= '<div class="swiper-wrapper">';
        foreach ($slides as $slide) {
            $galleryObjPos = isset($slide->image_position) && trim($slide->image_position) !== '' ? trim($slide->image_position) : 'center center';
            $output .= '<div class="swiper-slide" style="position:relative;"><div style="position:absolute;top:0;left:0;width:100%;height:100%;background-image:url(\'' . \e($slide->image_url) . '\');background-size:cover;background-position:' . \e($galleryObjPos) . ';background-repeat:no-repeat;"></div></div>';
        }
        $output .= '</div>';
    ...
    public static function processShortcodes(?string $content): string
    {
        ...
        // PASO 4: Protect ALL spacing paragraphs (with or without style attributes)
        // Convert <p>&nbsp;</p> and <p style="...">&nbsp;</p> and <p><span...>&nbsp;</span></p> to div spacers
        $content = preg_replace('/<p[^>]*>(\s*(?:&nbsp;|\xC2\xA0)\s*)<\/p>/i', '<div class="editor-spacing" style="height:24px;clear:both;"></div>', $content);
        // Also handle <p><span...>&nbsp;</span></p> (spacing with inline styles like bold/color)
        $content = preg_replace('/<p[^>]*>\s*<(?:span|strong|em|b|i)[^>]*>\s*(?:&nbsp;|\xC2\xA0)\s*<\/(?:span|strong|em|b|i)>\s*<\/p>/i', '<div class="editor-spacing" style="height:24px;clear:both;"></div>', $content);
        // Handle nested: <p><span><strong>&nbsp;</strong></span></p>
        $content = preg_replace('/<p[^>]*>\s*<span[^>]*>\s*<strong>\s*(?:&nbsp;|\xC2\xA0)\s*<\/strong>\s*<\/span>\s*<\/p>/i', '<div class="editor-spacing" style="height:24px;clear:both;"></div>', $content);

        // PASO 5: Limpiar <p>...</p> residuales que envolvían shortcodes
