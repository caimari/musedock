...
class SuperadminPluginService
{
    ...
    /**
     * Obtener todos los directorios donde buscar plugins de superadmin.
     * Incluye el directorio local y el externo (PRIVATE_PLUGINS_PATH) si existe.
     */
    public static function getPluginsDirs(): array
    {
        $dirs = [self::$pluginsDir];

        $privatePath = function_exists('env') ? env('PRIVATE_PLUGINS_PATH') : ($_ENV['PRIVATE_PLUGINS_PATH'] ?? null);
        if ($privatePath) {
            $externalDir = rtrim($privatePath, '/') . '/superadmin';
            if (is_dir($externalDir) && $externalDir !== self::$pluginsDir) {
                $dirs[] = $externalDir;
            }
        }
    ...
    public static function scanPlugins(): array
    {
        ...
        foreach (self::getPluginsDirs() as $dir) {
            ...
            foreach ($directories as $pluginDir) {
                $pluginInfo = self::getPluginInfo($pluginDir);
