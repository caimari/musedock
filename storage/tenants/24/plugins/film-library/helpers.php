<?php

/**
 * Film Library — Helper Functions
 */

if (!function_exists('film_url')) {
    function film_url(string $slug = '', string $type = 'film'): string
    {
        $base = '/films';

        if ($type === 'genre') {
            return $base . '/genero/' . $slug;
        }
        if ($type === 'director') {
            return $base . '/director/' . $slug;
        }
        if ($type === 'country') {
            return $base . '/country/' . $slug;
        }
        if ($type === 'year') {
            return $base . '/year/' . $slug;
        }
        if ($slug !== '') {
            return $base . '/' . $slug;
        }

        return $base;
    }
}

if (!function_exists('film_admin_url')) {
    function film_admin_url(string $path = ''): string
    {
        return admin_url('films' . ($path ? '/' . ltrim($path, '/') : ''));
    }
}

if (!function_exists('film_slugify')) {
    function film_slugify(string $text): string
    {
        $text = mb_strtolower($text, 'UTF-8');

        $map = [
            'á'=>'a','é'=>'e','í'=>'i','ó'=>'o','ú'=>'u',
            'ñ'=>'n','ü'=>'u','à'=>'a','è'=>'e','ì'=>'i',
            'ò'=>'o','ù'=>'u','â'=>'a','ê'=>'e','î'=>'i',
            'ô'=>'o','û'=>'u','ä'=>'a','ö'=>'o','ç'=>'c',
            'ß'=>'ss',
        ];
        $text = strtr($text, $map);

        $text = preg_replace('/[^a-z0-9]+/', '-', $text);
        return trim($text, '-');
    }
}

if (!function_exists('film_render_admin')) {
    function film_render_admin(string $template, array $data = []): string
    {
        \Screenart\Musedock\View::addGlobalData([]);

        $globalData = (new \ReflectionClass(\Screenart\Musedock\View::class))
            ->getProperty('globalData');
        $globalData->setAccessible(true);
        $globals = $globalData->getValue();
        $data = array_merge($globals, $data);

        if (isset($GLOBALS['ADMIN_MENU']) && !isset($data['ADMIN_MENU'])) {
            $data['ADMIN_MENU'] = $GLOBALS['ADMIN_MENU'];
        }

        $pluginViews = __DIR__ . '/views';
        $tenantLayout = dirname(__DIR__, 5) . '/core/Views/Tenant';
        $sharedViews = dirname(__DIR__, 5) . '/core/Views';
        $cache = dirname(__DIR__, 5) . '/storage/cache/film-library';

        if (!is_dir($cache)) {
            mkdir($cache, 0775, true);
        }

        $blade = new \Screenart\Musedock\BladeExtended(
            [$pluginViews, $tenantLayout, $sharedViews],
            $cache,
            config('debug', false)
                ? \Screenart\Musedock\BladeExtended::MODE_DEBUG
                : \Screenart\Musedock\BladeExtended::MODE_AUTO
        );

        $blade->directive('section', fn($arg) => "<?php \\Screenart\\Musedock\\View::startSection({$arg}); ?>");
        $blade->directive('endsection', fn() => "<?php \\Screenart\\Musedock\\View::stopSection(); ?>");
        $blade->directive('yield', fn($arg) => "<?php echo \\Screenart\\Musedock\\View::yieldSection({$arg}); ?>");
        $blade->directive('push', fn($arg) => "<?php \\Screenart\\Musedock\\View::startPush({$arg}); ?>");
        $blade->directive('endpush', fn() => "<?php \\Screenart\\Musedock\\View::stopPush(); ?>");
        $blade->directive('stack', fn($arg) => "<?php echo \\Screenart\\Musedock\\View::yieldPush({$arg}); ?>");
        $blade->directive('csrf', fn() => "<?php echo '<input type=\"hidden\" name=\"_token\" value=\"' . htmlspecialchars(csrf_token()) . '\">'; ?>");
        $blade->directive('menu', fn($arg) => "<?php echo \\Screenart\\Musedock\\Helpers\\MenuHelper::renderMenu({$arg}); ?>");
        $blade->directive('setting', fn($arg) => "<?php echo get_setting({$arg}); ?>");

        $modulesPath = dirname(__DIR__, 5) . '/modules';
        if (is_dir($modulesPath)) {
            foreach (scandir($modulesPath) as $mod) {
                $modViewPath = $modulesPath . '/' . $mod . '/views';
                if ($mod !== '.' && $mod !== '..' && is_dir($modViewPath)) {
                    $ns = str_replace(' ', '', ucwords(str_replace('-', ' ', $mod)));
                    $blade->addNamespace($ns, $modViewPath);
                }
            }
        }

        try {
            return $blade->run($template, $data);
        } catch (\Exception $e) {
            error_log("Film Library render error: " . $e->getMessage());
            if (config('debug', false)) {
                return "<div style='color:red;padding:20px'><h3>Film Library View Error</h3><p>" .
                    htmlspecialchars($e->getMessage()) . "</p><pre>" .
                    htmlspecialchars($e->getTraceAsString()) . "</pre></div>";
            }
            return "Error al renderizar vista.";
        }
    }
}

if (!function_exists('film_is_active')) {
    function film_is_active(): bool
    {
        $tenantId = function_exists('tenant_id') ? tenant_id() : null;
        if ($tenantId === null) {
            return false;
        }

        try {
            $pdo = \Screenart\Musedock\Database::connect();
            $stmt = $pdo->prepare("SELECT active FROM tenant_plugins WHERE tenant_id = ? AND slug = ? LIMIT 1");
            $stmt->execute([$tenantId, 'film-library']);
            $row = $stmt->fetch(\PDO::FETCH_ASSOC);
            return $row && $row['active'];
        } catch (\Exception $e) {
            return false;
        }
    }
}

if (!function_exists('film_poster_url')) {
    /**
     * Get poster/image URL respecting image_source setting.
     * - 'tmdb' (default): serve from TMDb CDN
     * - 'local': serve from local backup copy
     * - 'local_fallback': try local first, fallback to TMDb
     */
    function film_poster_url(?string $posterPath, string $size = 'w500', string $type = 'posters'): string
    {
        if (empty($posterPath)) {
            return '/assets/img/no-poster.svg';
        }
        // Already a full URL (manual upload or override)
        if (str_starts_with($posterPath, 'http') || str_starts_with($posterPath, '/storage/')) {
            return $posterPath;
        }

        $source = film_setting('image_source', 'tmdb');

        if ($source === 'local' || $source === 'local_fallback') {
            $tenantId = function_exists('tenant_id') ? tenant_id() : null;
            if ($tenantId) {
                $filename = $size . str_replace('/', '_', $posterPath);
                $localPath = '/storage/tenants/' . $tenantId . '/films/images/' . $type . '/' . $filename;
                $fullPath = dirname(__DIR__, 5) . $localPath;

                if (file_exists($fullPath) && filesize($fullPath) > 0) {
                    return $localPath;
                }
            }
            // If local_fallback or local file missing, fall through to TMDb
            if ($source === 'local') {
                return '/assets/img/no-poster.svg'; // strict local: no fallback
            }
        }

        return 'https://image.tmdb.org/t/p/' . $size . $posterPath;
    }
}

if (!function_exists('film_setting')) {
    function film_setting(string $key, $default = null)
    {
        static $settings = null;
        if ($settings === null) {
            try {
                $tenantId = function_exists('tenant_id') ? tenant_id() : null;
                if ($tenantId === null) return $default;

                $pdo = \Screenart\Musedock\Database::connect();
                $stmt = $pdo->prepare("SELECT settings FROM tenant_plugins WHERE tenant_id = ? AND slug = 'film-library' LIMIT 1");
                $stmt->execute([$tenantId]);
                $row = $stmt->fetch(\PDO::FETCH_ASSOC);
                $settings = $row ? (json_decode($row['settings'] ?? '{}', true) ?: []) : [];
            } catch (\Exception $e) {
                $settings = [];
            }
        }
        return $settings[$key] ?? $default;
    }
}
